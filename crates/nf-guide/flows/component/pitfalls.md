...
**症状**：按 flow 建议加了 `if (host._rendered && t > 1.0) return;` 防动画重播，但没效果。
...
**修复**：component flow 的 00-pick step 在"选 type=media"时**强制链接 pitfalls 坑 11**。
...
**说明**：v0.9.2 已完全移除 `particle` type，这条坑位只保留作历史记录，不再适用于现行 scene flow。
...
**症状**：项目有 `nf-guide component flow` + `.claude/skills/scene-dev/skill.md` 两套，AI 触发时用了旧的 skill（停在旧 scene type 世界观），写出来的组件和现行 flow 不一致。

**根因**：两处都能触发组件开发，但只维护了一处。

**修复**：删除 scene-dev skill，nf-guide 是唯一入口（ADR-008 更新指向 nf-guide）。

**防复发**：任何新状态机/flow 必须先查 nf-guide，不建平行目录。CLAUDE.md `src/nf-guide/` 已写"nf-guide is sole state-machine truth"。
