...
fn examples_for(topic: &str) -> Vec<&'static str> {
    match topic {
        ...
        "episodes" => vec![
            ...
            "nf episodes create --project=next-frame --slug=ep-02 --name='Episode 2'",
        ],
        ...
        "episodes rename" => {
            vec!["nf episodes rename --project=next-frame --episode=ep-01 --name='Intro Cut'"]
        }
        "episodes archive" => vec!["nf episodes archive --project=next-frame --episode=ep-01"],
        "episodes delete" => {
            vec!["nf episodes delete --project=next-frame --episode=ep-01 --confirm"]
        }
        "clips" => vec![
        ...
        "clips delete" => {
            vec!["nf clips delete --project=next-frame --episode=ep-01 --clip=intro --confirm"]
        }
        "anchors" => vec![
        ...
        "anchors set" => {
            vec!["nf anchors set --project=next-frame --episode=ep-01 --name=intro-end --time=5.0"]
        }
        "anchors unset" => {
            vec!["nf anchors unset --project=next-frame --episode=ep-01 --name=intro-end"]
        }
        "log" => vec![
        ...
        "doctor" => vec!["nf doctor", "nf doctor --human"],
        "open" => vec![
...
fn common_errors_for(topic: &str) -> Vec<&'static str> {
    match topic {
        "ps" => vec![
            "socket failed: hint: open a window with `nf open --project=<slug> --episode=<slug>`",
        ],
        "screenshot" => vec![
            "invalid region: hint: use full, topbar, clips, log, timeline, inspector, or preview",
        ],
        "click" => vec![
            "selector not found: hint: inspect DOM with `nf devtools --query=<sel> --get=shadowRoot`",
        ],
        ...
        "state" => vec![
            "unknown key: hint: try topbar.current-tab, clips.selected-id, timeline.current-time, or inspector.clip-id",
        ],
        "devtools" => {
            vec!["selector not found: hint: broaden selector, then narrow through ::shadow"]
        }
        ...
        "projects delete" => {
            vec!["missing --confirm: hint: rerun with --confirm only when intended"]
        }
        ...
        "episodes create" => {
            vec!["slug exists: hint: choose another episode slug or show the existing one"]
        }
        "episodes delete" => {
            vec!["missing --confirm: hint: rerun with --confirm only when intended"]
        }
        ...
        "anchors unset" => {
            vec!["anchor still referenced: hint: run nf clips update to move references first"]
        }
        "log" | "log tail" => {
            vec!["invalid limit or since: hint: pass --limit=<n> and ISO datetime"]
        }
        "log show" => {
            vec!["unknown log entry: hint: nf log tail --project=<slug> --episode=<slug>"]
        }
        "log create" => vec![
            "invalid actor/status: hint: actor is AI or human; status is pending, done, or failed",
        ],
