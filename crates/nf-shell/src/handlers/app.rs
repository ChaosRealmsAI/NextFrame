...
impl AppOpHandler {
    ...
    pub fn state_query(manager: &WindowManager, request: IpcRequest) -> IpcResponse {
        ...
        if query == Some("windows") {
            ...
            return ok_response(&request, json!({ "windows": windows, "count": windows.len() }));
        }
        ...
        let result = (|| {
            ...
            let window = optional_string(&request.params, "window");
            let (window_id, _) = manager.webview_for_target(&project, &episode, window.as_deref())?;
            Ok(json!({
                "window_id": window_id,
                "project": project,
                "episode": episode,
...
fn callback_for_shared(
    req_id: String,
    shared_ack: SharedAck,
...
fn js_callback_response(req_id: &str, raw: &str) -> IpcResponse {
    match serde_json::from_str::<Value>(raw) {
        ...
        Err(err) => IpcResponse {
            ...
            ok: false,
            data: None,
            error: Some(format!("invalid JavaScript callback JSON: {err}; raw={raw}")),
        },
    }
}
...
fn screenshot_response(
    ...
    if let Some(path) = output.as_ref() {
        if let Err(err) = write_png(path, &png) {
            return error_response(req_id.to_string(), format!("write screenshot failed: {err}"));
        }
    }
