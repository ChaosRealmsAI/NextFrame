import { createRequire as __nf_cr } from 'node:module'; const require = __nf_cr(import.meta.url);
...
function findCycleChain(graph, remaining) {
...
// src/types.ts
var StageErrorException = class extends Error {
  err;
  constructor(err) {
    super(err.message);
    this.err = err;
...
var validateSource = ajv.compile(SOURCE_SCHEMA);
function parseError(code, message, fix_hint, extra) {
  const err = { stage: "parse", code, message };
  if (fix_hint !== void 0) err.fix_hint = fix_hint;
  if (extra !== void 0) err.loc = extra;
  return new StageErrorException(err);
}
function isPointAnchor(a) {
  return typeof a.at === "string";
}
function parseSource(text, file) {
  ...
  const anchorSources = source.anchors ?? {};
  for (const [name, a] of Object.entries(anchorSources)) {
    try {
      if (isPointAnchor(a)) {
        const at = parseExpr(a.at);
        const pa = { name, kind: "point", exprs: { at } };
        if (a.filler !== void 0) pa.filler = a.filler;
        anchors.set(name, pa);
      } else {
        const begin = parseExpr(a.begin);
        const end = parseExpr(a.end);
        const ra = { name, kind: "range", exprs: { begin, end } };
        if (a.filler !== void 0) ra.filler = a.filler;
        anchors.set(name, ra);
      }
    } catch (e) {
      if (e instanceof ExprParseError) {
        throw parseError("E_EXPR_SYNTAX", `anchor '${name}': ${e.message}`, "Fix expr; see PEG grammar (interfaces \xA73).", {
          anchor_name: name,
          expr: e.expr,
          col: e.col
        });
  ...
  for (const [name, pa] of anchors.entries()) {
    ...
    for (const ast of Object.values(pa.exprs)) {
      if (ast) {
        for (const r of collectRefs(ast)) if (r !== name) deps.add(r);
      }
    }
    refGraph[name] = [...deps];
  }
  const anchorNames = new Set(anchors.keys());
  const checkRefs = (ast, ctx) => {
    if (!ast) return;
...
function resolveError(code, message, fix_hint, loc) {
  const err = { stage: "resolve", code, message };
  if (fix_hint !== void 0) err.fix_hint = fix_hint;
  if (loc !== void 0) err.loc = loc;
  return new StageErrorException(err);
}
function evalExpr(ast, resolvedAnchors, context) {
  ...
  if (!ra) {
    const loc = {};
    if (context.anchor_name !== void 0) loc.anchor_name = context.anchor_name;
    if (context.clip_id !== void 0) loc.clip_id = context.clip_id;
...
function resolveAnchor(pa, resolvedAnchors) {
  if (pa.kind === "point") {
...
function resolve(parsed, loadDescribe) {
  ...
  for (const track of parsed.tracks) {
    ...
    if (descriptor && descriptor.params) {
      ...
      if (!v) {
        try {
          v = ajv2.compile(descriptor.params);
        } catch (e) {
          throw resolveError(
            "E_TRACK_SCHEMA",
  ...
  if (parsed.raw.meta !== void 0) out.meta = parsed.raw.meta;
  return out;
}
...
function replaceIdentInExpr(expr, from, to) {
  ...
  let changed = 0;
  const out = expr.replace(re, (_m, pre) => {
    changed++;
    return `${pre}${to}`;
  });
  return { out, changed };
...
function rename(source_text, old_name, new_name) {
...
function resolveTrackPath(relativeSrc, cwdDir) {
...
function resolveError2(code, message, fix_hint) {
  const err = { stage: "resolve", code, message };
  if (fix_hint !== void 0) err.fix_hint = fix_hint;
  return new StageErrorException(err);
}
function executeTrackSrc(src, trackId) {
  const transformed = src.replace(/export\s+function\s+([A-Za-z_$][\w$]*)/g, "exports.$1 = function $1").replace(/export\s+const\s+([A-Za-z_$][\w$]*)/g, "exports.$1 = /* const */").replace(/export\s+default\s+/g, "exports.default = ");
  const exports = {};
  ...
  try {
    ...
    throw resolveError2("E_TRACK_EXEC", `track '${trackId}' failed to execute: ${e instanceof Error ? e.message : String(e)}`, "Track must be pure ES module with exports.");
  }
  return exports;
}
function loadTrack(relativeSrc, trackId, cwdDir) {
  const fullPath = resolveTrackPath(relativeSrc, cwdDir);
  if (!fullPath) {
    throw resolveError2("E_TRACK_READ", `cannot locate track '${trackId}' with src='${relativeSrc}' (searched ${cwdDir} and ancestors)`, "Check path in source.json.");
  }
  let text;
  try {
    text = readFileSync(fullPath, "utf8");
  } catch (e) {
    throw resolveError2("E_TRACK_READ", `cannot read track '${trackId}' at ${fullPath}: ${e instanceof Error ? e.message : String(e)}`, "Check path in source.json.");
  }
  ...
  if (typeof mod.describe === "function") {
    try {
      ...
      throw resolveError2("E_TRACK_DESCRIBE", `track '${trackId}' describe() threw: ${e instanceof Error ? e.message : String(e)}`, "Ensure describe() is pure.");
    }
  }
  return {
    id: trackId,
...
function loadTracksFor(tracks, cwdDir) {
...
function emitError(stage, code, message, fix_hint, loc) {
  const err = { stage, code, message };
  if (fix_hint !== void 0) err.fix_hint = fix_hint;
  if (loc !== void 0) err.loc = loc;
  emit({ error: err });
}
...
function makeLoader(parsed, sourceDir) {
  ...
  const simplified = /* @__PURE__ */ new Map();
...
function handleCmd(input) {
  try {
    switch (input.cmd) {
      ...
      case "anchors":
        return doAnchors(input.args);
      case "rename-anchor":
        return doRename(input.args);
      case "load-track-describe":
        return doLoadTrack(input.args);
      case "ping":
...
function doRename(args) {
  ...
  emit({ event: "rename.done", from, to, changed_locations, new_source: write ? void 0 : new_source });
}
function doLoadTrack(args) {
  const source = String(args.source ?? "");
  if (!source) throw new StageErrorException({ stage: "parse", code: "E_ARGS", message: "missing 'source' path" });
  const text = loadSourceText(source);
  const parsed = parseSource(text, source);
...
var cmdIdx = argv.indexOf("--cmd");
if (cmdIdx >= 0 && argv[cmdIdx + 1] !== void 0) {
