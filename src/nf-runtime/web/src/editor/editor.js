...
function updateEditorPreviewState(currentTime, totalDuration) {
    ...
    if (previewTc)
        previewTc.textContent = editorClock(time) + ' / ' + editorClock(duration);
    const transportTc = document.querySelector('.ed-transport-tc');
    if (transportTc)
        transportTc.textContent = editorClock(time);
...
async function ensureEditorSceneBundle(timeline) {
    ...
    if (!url)
        throw new Error('Missing preview bundle URL');
...
function resolveEditorSelectionIndex(selection) {
    if (typeof selection === 'number')
        return selection;
    if (!selection || !edTimelineData || !Array.isArray(edTimelineData.layers))
        return null;
    ...
    const clipId = typeof selection === 'string'
        ? selection
        : (selObj.id || selObj.layerId || selObj.sceneId || selObj.scene || '');
    if (!clipId)
        return null;
...
function selectTimelineClip(index) {
    if (!edTimelineData || !Array.isArray(edTimelineData.layers) || !edTimelineData.layers[index])
        return;
    edActiveClip = index;
...
function handleTimelineTrackClick(event) {
    ...
    const clipEl = target && target.closest ? target.closest('.ed-tl-clip[data-index]') : null;
    if (!clipEl || !edTimelineData)
        return;
...
function updatePreviewScale() {
    ...
    if (!canvas || !stage || !edTimelineData)
        return;
...
async function composePreview() {
    ...
    if (typeof window.bindPreviewStateSource === 'function')
        window.bindPreviewStateSource();
    toggleEditorPreviewPlaceholder(false);
...
function edTimelineSeekFromX(clientX) {
    ...
    if (typeof window.sendPreviewCmd === 'function')
        window.sendPreviewCmd('seek', time);
    else
        previewFrame(time);
}
(function wireTimelineInteraction() {
    ...
    if (ruler)
        ruler.addEventListener('click', function (e) { edTimelineSeekFromX(e.clientX); });
    ...
    if (hitArea) {
        ...
        document.addEventListener('pointermove', function (e) { if (dragging)
            edTimelineSeekFromX(e.clientX); });
