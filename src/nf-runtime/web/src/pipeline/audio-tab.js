...
function renderAudioTab(segments) {
    ...
    audioSegments.forEach(function (seg, index) {
        ...
        if (!pipelineAudioState[segmentNumber] && typeof bridgeCall === 'function' && getCurrentEpisodeRef()) {
            bridgeCall('audio.status', { episode: getCurrentEpisodeRef(), segment: segmentNumber }).then(function (result) {
...
function renderAudioTabInner(segments) {
    ...
    if (sidebar) {
        ...
        segments.forEach(function (seg, index) {
            ...
            if (pipelineAudioState[sn] && pipelineAudioState[sn].exists)
                generatedCount++;
        });
    ...
    segments.forEach(function (seg, index) {
        ...
        const state = pipelineAudioState?.[segmentNumber] || { exists: false };
        const hasAudio = state.exists && state.mp3;
        const tl = state.timelineData;
        const sentences = (tl && Array.isArray(tl.segments) ? tl.segments : []);
        const duration = seg.duration || (sentences.length > 0 ? Number(sentences[sentences.length - 1].end_ms) / 1000 : 0);
        ...
        if (duration > 0)
            html += '<span style="font-family:var(--mono);font-size:13px;color:var(--t80);margin-left:auto">' + duration.toFixed(1) + 's</span>';
        html += '</div>';
        // File path
        ...
        if (hasAudio && sentences.length > 0) {
            ...
            sentences.forEach(function (sent) {
                ...
                const wordSpans = words.length > 0
                    ? words.map(function (w) { return '<span class="ch" data-start="' + (w.start_ms || 0) + '" data-end="' + (w.end_ms || 0) + '">' + escapeHtml(String(w.word || w.char || '')) + '</span>'; }).join('')
...
function generateTTS(segmentNumber) {
    if (typeof bridgeCall !== 'function' || !getCurrentEpisodeRef())
        return;
