...
interface ScLanguageRow {
  label: string;
  text: string;
  primary: boolean;
}

interface ScWordModel {
  text: string;
  startSec: number;
  endSec: number;
}

interface ScSentenceModel {
  id: number;
  startSec: number;
  endSec: number;
  absStartSec?: number;
  ...
  text: string;
  words: ScWordModel[];
  languageRows: ScLanguageRow[];
}

interface ScClipModel {
  index: number;
  clipNum: number;
  title: string;
  fileName: string;
  ...
  textPreview: string;
  why: string;
  sentences: ScSentenceModel[];
  words: ScWordModel[];
  rawClip: Record<string, unknown>;
  rawPlan: Record<string, unknown>;
}

interface ScSourceModel {
  index: number;
  slug: string;
  dirPath: string;
  path: string;
  ...
  words: ScWordModel[];
  clips: ScClipModel[];
  metaProbeStarted?: boolean;
}

interface ScPlaybackConfig {
  title: string;
  sentences: ScSentenceModel[];
  durationSec: number;
  seekSec?: number;
...
let scSources: ScSourceModel[]=[]; let scActiveSource=0; let scRenderedClips: ScClipModel[]=[]; let scPlayerCleanup: (() => void) | null=null; let scLoadToken=0;

function scRenderEmptySubtitle(text: string){return'<div class="sc-modal-sub-lang-row"><span class="sc-modal-sub-original">'+escapeHtml(text||'···')+'</span></div>';}

function scExtractLanguageRows(sentence: Record<string, unknown>): ScLanguageRow[]{
  const rows: ScLanguageRow[]=[]; const translations=sentence&&sentence.translations&&typeof sentence.translations==='object'?sentence.translations as Record<string, unknown>:{} as Record<string, unknown>;
  if(sentence&&sentence.text)rows.push({label:'EN',text:String(sentence.text),primary:true});
  ['zh','ja','ko','fr','es','de'].forEach((key)=>{const text=translations[key]||sentence[key]||''; if(text)rows.push({label:langLabel(key),text:String(text),primary:false});});
  return rows;
}

function scWordStart(word: Record<string, unknown>){return Number(word&&((word.startSec!=null)?word.startSec:word.start));}
function scWordEnd(word: Record<string, unknown>){return Number(word&&((word.endSec!=null)?word.endSec:word.end));}

function scNormalizeWord(word: Record<string, unknown>,timeOffset: number){
...
function scReadJson<T>(path: string,fallback: T): Promise<T>{
  if(typeof bridgeCall!=='function'||!path)return Promise.resolve(fallback);
  return bridgeCall<Record<string, unknown>>('fs.read',{path}).then((result: NfBridgeResult<Record<string, unknown>>) => {const data=result.ok===true?result.value:{} as Record<string, unknown>; try{return JSON.parse(data&&data.contents?data.contents as string:'null')||fallback;}catch(_error){return fallback;}}).catch(()=>fallback);
}

function scListDir(path: string){
  if(typeof bridgeCall!=='function'||!path)return Promise.resolve([] as Record<string, unknown>[]);
  return bridgeCall<Record<string, unknown>>('fs.listDir',{path}).then((result: NfBridgeResult<Record<string, unknown>>) => {const data=result.ok===true?result.value:{} as Record<string, unknown>; return Array.isArray(data&&data.entries)?data.entries as Record<string, unknown>[]:[]; }).catch(()=>[] as Record<string, unknown>[]);
}

function scFindSourceVideo(entries: Record<string, unknown>[]){
  return entries.find((entry: Record<string, unknown>) => /\.(mp4|mov|mkv|webm)$/i.test(String(entry.name||'')))||entries.find((entry: Record<string, unknown>) => String(entry.name||'')==='source.mp4')||null;
}

function scBuildSourceSentences(data: Record<string, unknown>,words: Record<string, unknown>): ScSentenceModel[]{
  const rawSentences=(Array.isArray(data&&data.sentences)?data.sentences:[]) as Record<string, unknown>[]; const rawWords=(Array.isArray(words&&words.words)?words.words:[]) as Record<string, unknown>[];
  return rawSentences.map((sentence: Record<string, unknown>) => {
    const startSec=Number(sentence&&sentence.start); const endSec=Number(sentence&&sentence.end);
    const sentenceWords=((Array.isArray(sentence&&sentence.words)?sentence.words as Record<string, unknown>[]:rawWords.filter((word: Record<string, unknown>) => Number(word&&word.end)>startSec&&Number(word&&word.start)<endSec)) as Record<string, unknown>[]).map((word: Record<string, unknown>) => scNormalizeWord(word,0));
    return{
...
function scBuildSourceModel(dirEntry: Record<string, unknown>,fileEntries: Record<string, unknown>[],meta: Record<string, unknown>,sentencesData: Record<string, unknown>,wordsData: Record<string, unknown>,planTitle: string,index: number){
  const videoEntry=scFindSourceVideo(fileEntries); const videoPath=videoEntry?String(videoEntry.path):'';
  const words=(Array.isArray(wordsData&&wordsData.words)?wordsData.words:[]) as ScWordModel[];
  ...
  return{
    ...
    totalWords:Number(wordsData&&wordsData.total_words)||(words as unknown[]).length,
    sentences,
    words,
    clips:[] as ScClipModel[],
  } as ScSourceModel;
}

function scSourceOwnsClip(source: ScSourceModel,clip: Record<string, unknown>){
  ...
  const first=source.sentences.find((sentence: ScSentenceModel) => sentence.id===fromId);
  const last=source.sentences.find((sentence: ScSentenceModel) => sentence.id===toId);
  ...
  return Number.isFinite(startSec)&&Number.isFinite(endSec)&&source.sentences.some((sentence: ScSentenceModel) => sentence.endSec>startSec&&sentence.startSec<endSec);
}

function scBuildClipModel(source: ScSourceModel,clip: Record<string, unknown>,fileMeta: Record<string, unknown> | null,planMeta: Record<string, unknown> | null,index: number){
  const startSec=Number(clip&&clip.start); const endSec=Number(clip&&clip.end);
  const clipSentences=source.sentences.filter((sentence: ScSentenceModel) => sentence.id>=Number(clip&&clip.from_id)&&sentence.id<=Number(clip&&clip.to_id));
  const clipWords=source.words.filter((word: ScWordModel) => word.endSec>startSec&&word.startSec<endSec).map((word: ScWordModel) => scNormalizeWord(word as unknown as Record<string, unknown>,startSec));
  const sentences=clipSentences.map((sentence: ScSentenceModel) => ({
    id:sentence.id,
    ...
    text:sentence.text,
    words:(sentence.words.length?sentence.words:source.words.filter((word: ScWordModel) => word.endSec>sentence.startSec&&word.startSec<sentence.endSec)).map((word: ScWordModel) => scNormalizeWord(word as unknown as Record<string, unknown>,startSec)),
...
function scAttachClipsToSources(sources: ScSourceModel[],cutReport: Record<string, unknown>,plan: Record<string, unknown>,clipFiles: Record<string, unknown>[]){
  const success=(Array.isArray(cutReport&&cutReport.success)?cutReport.success:[]) as Record<string, unknown>[]; const planClips=(Array.isArray(plan&&plan.clips)?plan.clips:[]) as Record<string, unknown>[]; const clipFileMap: Record<string, Record<string, unknown>>={};
  clipFiles.forEach((clip: Record<string, unknown>) => {clipFileMap[String(clip.name)]=clip;});
  sources.forEach((source: ScSourceModel,index: number)=>{source.title=formatSourceTitle(source as unknown as NfSource,index); source.durationLabel=formatClock(source.durationSec,false); source.clips=[];});
  success.forEach((clip: Record<string, unknown>) => {
    const matchingSource=sources.find((source: ScSourceModel) => scSourceOwnsClip(source,clip))||(sources.length===1?sources[0]:null); if(!matchingSource)return;
    const planMeta=planClips.find((item: Record<string, unknown>) => Number(item&&item.from)===Number(clip&&clip.from_id)&&Number(item&&item.to)===Number(clip&&clip.to_id))||planClips.find((item: Record<string, unknown>) => Number(item&&item.id)===Number(clip&&clip.clip_num))||null;
  ...
  sources.forEach((source: ScSourceModel) => {source.clips.sort((left: ScClipModel,right: ScClipModel)=>left.startSec-right.startSec||left.clipNum-right.clipNum); source.clips.forEach((clip: ScClipModel,index: number)=>{clip.index=index;});});
  return sources;
}

function scWarmSourceMetadata(source: ScSourceModel){
...
function scRenderTimeline(clips: ScClipModel[],source: ScSourceModel){
  ...
  const regions=clips.map((clipModel: ScClipModel) => {
...
function scRenderClipSentence(clipIndex: number,sentence: ScSentenceModel,sentenceIndex: number){
  const rows=(sentence.languageRows&&sentence.languageRows.length?sentence.languageRows:[{label:'EN',text:sentence.text||'字幕缺失',primary:true}] as ScLanguageRow[]).filter((row: ScLanguageRow) => row.primary||row.text).map((row: ScLanguageRow) => '<div class="sc-sent-lang-row"><span class="sc-sent-lang-label">'+escapeHtml(row.label)+'</span><span class="'+(row.primary?'sc-sent-text':'sc-sent-trans')+'">'+escapeHtml(row.text)+'</span></div>').join('');
...
function scRenderMain(){
  ...
  const cardsHtml=scRenderedClips.length?scRenderedClips.map((clipModel: ScClipModel) => '<div class="sc-clip-card'+(clipModel.index===0?' active':'')+'" id="sc-clip-'+clipModel.index+'">'+
...
function scRenderCurrentSentence(subtitleEl: HTMLElement | null,sentence: ScSentenceModel | null,emptyText: string): {words: ScWordModel[]; spans: Element[]; states: string[]; key: string}{
  const emptyResult: {words: ScWordModel[]; spans: Element[]; states: string[]; key: string}={words:[],spans:[],states:[],key:''};
  if(!subtitleEl)return emptyResult;
  if(!sentence){subtitleEl.innerHTML=scRenderEmptySubtitle(emptyText); return emptyResult;}
  const rows=(sentence.languageRows&&sentence.languageRows.length?sentence.languageRows:[{label:'EN',text:sentence.text||'',primary:true}] as ScLanguageRow[]).filter((row: ScLanguageRow) => row.primary||row.text);
  const primaryRow=rows.find((row: ScLanguageRow) => row.primary)||rows[0]||{label:'EN',text:sentence.text||'',primary:true};
  const translationRows=rows.filter((row: ScLanguageRow) => row!==primaryRow&&!row.primary&&row.text);
  const words: ScWordModel[]=(Array.isArray(sentence.words)&&sentence.words.length?sentence.words:[{text:primaryRow.text||sentence.text||'···',startSec:sentence.startSec,endSec:sentence.endSec}]);
  subtitleEl.innerHTML='<div class="sc-modal-sub-lang-row"><span class="sc-modal-sub-lang-label">'+escapeHtml(primaryRow.label||'EN')+'</span><span class="sc-modal-sub-original">'+words.map((word: ScWordModel) => '<span class="sc-modal-word sc-word-pending">'+escapeHtml(word.text)+'</span>').join(' ')+'</span></div>'+translationRows.map((row: ScLanguageRow) => '<div class="sc-modal-sub-lang-row"><span class="sc-modal-sub-lang-label">'+escapeHtml(row.label)+'</span><span class="sc-modal-sub-trans">'+escapeHtml(row.text)+'</span></div>').join('');
...
function scBindPlayer(video: HTMLVideoElement | null,playback: ScPlaybackConfig){
  ...
  const v=video; const curEl=currentEl; const totEl=totalEl; const fEl=fillEl; const pEl=progressEl;
  titleEl.textContent=playback.title||'';
  let subtitleState: {words: ScWordModel[]; spans: Element[]; states: string[]; key: string}={words:[],spans:[],states:[],key:''}; let rafId=0; let pendingSeek=Number.isFinite(playback.seekSec)?playback.seekSec:null;

  function activeSentenceAt(current: number){return playback.sentences.find((sentence: ScSentenceModel) => current>=sentence.startSec&&current<=sentence.endSec)||null;}

  function updateWordClasses(current: number){
    ...
    subtitleState.words.forEach((word,index)=>{
      const nextClass: 'sc-word-done'|'sc-word-now'|'sc-word-pending'=word.endSec<current?'sc-word-done':(word.startSec<=current&&current<=word.endSec?'sc-word-now':'sc-word-pending');
      if(subtitleState.states[index]!==nextClass){subtitleState.states[index]=nextClass; (subtitleState.spans[index] as HTMLElement).className='sc-modal-word '+nextClass;}
    });
  }

  function sync(){
    const duration=Number.isFinite(v.duration)&&v.duration>0?v.duration:(Number(playback.durationSec)||0); const current=Number.isFinite(v.currentTime)?v.currentTime:0;
    curEl.textContent=formatClock(current,true); totEl.textContent=formatClock(duration,true); fEl.style.width=duration?((current/duration)*100).toFixed(2)+'%':'0%';
  ...
  function onLoaded(){const duration=Number.isFinite(v.duration)&&v.duration>0?v.duration:(Number(playback.durationSec)||0); if(pendingSeek!=null){v.currentTime=Math.max(0,Math.min(duration||pendingSeek,pendingSeek)); pendingSeek=null;} sync();}
  function onProgressClick(event: MouseEvent){const rect=pEl.getBoundingClientRect(); const ratio=Math.min(1,Math.max(0,(event.clientX-rect.left)/rect.width)); const duration=Number.isFinite(v.duration)&&v.duration>0?v.duration:(Number(playback.durationSec)||0); if(duration)v.currentTime=ratio*duration;}

  v.addEventListener('loadedmetadata',onLoaded); v.addEventListener('seeked',sync); v.addEventListener('pause',sync); v.addEventListener('timeupdate',sync);
  pEl.addEventListener('click',onProgressClick); sync(); tick();
...
function scOpenPlayer(index: number,seekSec?: number){
  ...
  overlay.classList.add('open'); scBindPlayer(document.getElementById('sc-modal-video') as HTMLVideoElement | null,{title:clipModel.title,sentences:clipModel.sentences,durationSec:clipModel.durationSec,seekSec:Number(seekSec),emptyText:'···'});
}

function scOpenClipSentence(clipIndex: number,sentenceIndex: number){
...
function scOpenSourcePlayer(seekSec?: number){
  ...
  overlay.classList.add('open'); scBindPlayer(document.getElementById('sc-modal-video') as HTMLVideoElement | null,{title:source.title+' · '+(source.resolutionLabel!=='—'?source.resolutionLabel:source.formatLabel),sentences:source.sentences,durationSec:source.durationSec,seekSec:Number(seekSec),emptyText:'原视频预览'});
}
...
function scOpenMeta(index: number){
  ...
  const metaObject: Record<string, unknown>={title:clipModel.title,why:clipModel.why,source_sentence_range:clipModel.sentenceRange,source_timecode:clipModel.timecodeLabel,duration:formatClock(clipModel.durationSec,true),file_size:clipModel.sizeLabel,file:clipModel.fileName,path:clipModel.path,text_preview:clipModel.textPreview};
...
function scHandlePlayerOverlay(event: MouseEvent){if(event.target&&(event.target as HTMLElement).id==='sc-player-modal')scClosePlayer();}
function scHandleMetaOverlay(event: MouseEvent){if(event.target&&(event.target as HTMLElement).id==='sc-meta-modal')scCloseMeta();}
function scSelectSource(index: number){scActiveSource=index; scRenderMain();}
...
async function loadSmartClips(){
  ...
  const [sourceEntries,cutReport,plan,clipResult]=await Promise.all([
    ...
    bridgeCall<Record<string, unknown>>('source.clips',{episode:episodePath}).then((r: NfBridgeResult<Record<string, unknown>>)=>r.ok===true?r.value:{clips:[]}).catch(()=>({clips:[] as unknown[]})),
  ]);
  if(token!==scLoadToken)return;
  const dirs=sourceEntries.filter((entry: Record<string, unknown>) => entry&&entry.isDir&&!String(entry.name||'').startsWith('.')); const planTitle=typeof(plan&&(plan as Record<string, unknown>).source)==='string'?(plan as Record<string, unknown>).source as string:'';
  const sources=(await Promise.all(dirs.map(async (dirEntry: Record<string, unknown>,index: number)=>{
    const fileEntries=await scListDir(String(dirEntry.path)); const [meta,sentencesData,wordsData]=await Promise.all([
  ...
  if(token!==scLoadToken)return;
  const clipResultObj=clipResult as Record<string, unknown>; scSources=scAttachClipsToSources(sources,cutReport as Record<string, unknown>,plan as Record<string, unknown>,Array.isArray(clipResultObj&&clipResultObj.clips)?clipResultObj.clips as Record<string, unknown>[]:[]); scActiveSource=0; scRenderedClips=[];
  ...
  if(!scSources[0].title)scSources[0].title=formatSourceTitle(scSources[0] as unknown as NfSource,0);
  scSources.forEach((source: ScSourceModel,index: number)=>{if(!source.title)source.title=formatSourceTitle(source as unknown as NfSource,index); if(!source.durationLabel||source.durationLabel==='—')source.durationLabel=formatClock(source.durationSec,false); if(source.meta&&typeof source.meta.format==='string'&&source.meta.format.trim())source.formatLabel=source.meta.format.trim();});
...
async function scLoadClipTranslations(episodePath: string){
  const clipsDir=episodePath+'/clips';
  let entries: Record<string, unknown>[]=[]; try{const list=await bridgeCall('fs.listDir',{path:clipsDir}); const listVal=list.ok===true?list.value as Record<string,unknown>:null; entries=Array.isArray(listVal?.entries)?listVal.entries as Record<string, unknown>[]:[]; }catch(_e){}
  const availableLangs=new Set<string>(); entries.forEach((entry: Record<string, unknown>) => {const m=String(entry&&entry.name||'').match(/^clip_\d+\.translations\.([a-z]{2})\.json$/i); if(m)availableLangs.add(m[1].toLowerCase());});
  if(!availableLangs.size)return;
  await Promise.all(scSources.flatMap((source)=>
    source.clips.map(async (clip: ScClipModel) => {
      const pad=String(clip.clipNum).padStart(2,'0');
      for(const lang of availableLangs){
        const path=clipsDir+'/clip_'+pad+'.translations.'+lang+'.json';
        const data=await scReadJson<Record<string, unknown> | null>(path,null);
        if(!data||!Array.isArray(data.segments))continue;
        const label=langLabel(String(lang));
        (data.segments as Record<string, unknown>[]).forEach((seg: Record<string, unknown>) => {
          const sentence=clip.sentences.find((s: ScSentenceModel) => s.id===seg.id);
          if(!sentence)return;
          const cnCues=Array.isArray(seg.cn)?seg.cn:[];
          const text=cnCues.map((cue: unknown) => typeof cue==='string'?cue:String((cue as Record<string, unknown>)&&(cue as Record<string, unknown>).text||'')).join('');
          if(!text)return;
          const existing=sentence.languageRows.find((r: ScLanguageRow) => r.label===label);
