class Topbar extends Component {
  ...
  render() {
    ...
    return h(
      'header',
      { class: 'topbar', id: 'global-topbar' },
      h(
        'div',
        { class: 'tb-traffic-lights' },
        h('div', { class: 'tb-dot tb-dot--close' }),
        h('div', { class: 'tb-dot tb-dot--minimize' }),
        h('div', { class: 'tb-dot tb-dot--zoom' }),
      ),
      h(
        'div',
        {
        ...
        this.icon('<svg width="18" height="18" viewBox="0 0 18 18" fill="none"><rect x="1" y="1" width="16" height="16" rx="4" stroke="#a78bfa" stroke-width="1.5" fill="none"/><polygon points="7,5 13,9 7,13" fill="#a78bfa"/></svg>'),
        ' NextFrame ',
      ),
      h(
        'nav',
        { class: 'tb-breadcrumb', id: 'global-breadcrumb', style: { display: showBreadcrumb ? 'flex' : 'none' } },
        h('span', { class: 'tb-bc-sep' }, '/'),
        h(
          'span',
          {
      ...
      h(
        'div',
        { class: 'tb-pipeline-tabs', id: 'global-pl-tabs', style: { display: showPipelineTabs ? 'flex' : 'none' } },
        this.renderPipelineTab('script', '脚本'),
        this.renderPipelineTab('audio', '音频'),
        this.renderPipelineTab('clips', '智能切片'),
        this.renderPipelineTab('atoms', '原子'),
        this.renderPipelineTab('assembly', '智能剪辑'),
        this.renderPipelineTab('output', '产出'),
      ),
