...
let newProjectModalComponent = null;
...
function switchTabByStage(stage) {
...
function showView(viewName, data) {
    ...
    if (viewName === 'project') {
        ...
        if (nameEl)
            nameEl.textContent = window.currentProjectName;
    ...
    if (viewName === 'pipeline') {
        ...
        if (typeof loadPipelineData === 'function')
            loadPipelineData();
        if (typeof loadEditorTimeline === 'function')
            loadEditorTimeline();
        if (typeof loadSmartClips === 'function')
            loadSmartClips();
    }
...
function toggleSettings() {
    ...
    if (settingsPanelComponent)
        settingsPanelComponent.update({ open: next });
}
function toggleNewProject() {
    if (newProjectModalComponent && newProjectModalComponent.toggle)
        newProjectModalComponent.toggle();
}
function mountComponent(rootId, instance) {
