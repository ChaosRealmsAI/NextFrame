...
(function() {
  ...
  function applySelection() {
    ...
    state.stageEl.querySelectorAll('.nf-layer').forEach(function(el: any) {
  ...
  function compose(t: any) {
    ...
    if (state.stageEl) {
      ...
      existing.forEach(function(el: any) {
  ...
  function primeVideoFrame(video: any, t: any) {
  ...
  function syncVideos(t: any, playing: any) {
    if (!state.stageEl) return;
    state.stageEl.querySelectorAll('video[data-nf-persist]').forEach(function(v: any) {
      if (playing) {
  ...
  function loadTimeline(json: any) {
    const source = typeof json === 'string' ? JSON.parse(json) : (json || {});
    const layers = Array.isArray(source.layers) ? source.layers.slice() : [];
    const duration = Number.isFinite(source.duration) ? source.duration : layers.reduce(function(maxEnd: any, layer: any) {
