...
pub(crate) fn send_key_to_webview(
    ...
    if let Some(event) = &down
        && let Err(err) /* Internal: Objective-C responder failure is logged locally below */ = catch_objc(|| {
            ...
            let _: () = unsafe { msg_send![webview, keyDown: &**event] }; // SAFETY: `webview` is a live WKWebView and `keyDown:` is a valid responder selector for the synthesized NSEvent.
        })
    {
    ...
    if let Some(event) = &up
        && let Err(err) /* Internal: Objective-C responder failure is logged locally below */ = catch_objc(|| {
            ...
            let _: () = unsafe { msg_send![webview, keyUp: &**event] }; // SAFETY: `webview` is a live WKWebView and `keyUp:` is a valid responder selector for the synthesized NSEvent.
        })
    {
...
pub(crate) fn type_text_native(webview: &WKWebView, text: &str, result_path: &str) {
    ...
    for (i, ch) in chars.into_iter().enumerate() {
        ...
        dispatch::Queue::main().exec_after(std::time::Duration::from_millis(delay_ms), move || {
            ...
            let wv = unsafe { &*(wv_ptr as *const WKWebView) }; // SAFETY: `wv_ptr` was captured from a live WKWebView owned by app state and this work runs back on the main queue before the tab is torn down.
...
pub(crate) fn add_tag(webview: &WKWebView, tag: &str, result_path: &str) {
    ...
    let handler = RcBlock::new(move |result: *mut AnyObject, _error: *mut NSError| {
        let coords = if result.is_null() {
            ...
            let s: &NSString = unsafe { &*(result as *const NSString) }; // SAFETY: this JavaScript callback returns a string result, so non-null `result` is an NSString.
            s.to_string()
        };
        ...
        let wv = unsafe { &*(wv_ptr as *const WKWebView) }; // SAFETY: `wv_ptr` was captured from a live WKWebView owned by app state and this handler runs on the main queue.
        ...
        for (i, ch) in chars.into_iter().enumerate() {
            ...
            dispatch::Queue::main().exec_after(std::time::Duration::from_millis(cmd_delay), {
                ...
                move || {
                    ...
                    let wv = unsafe { &*(wv_ptr as *const WKWebView) }; // SAFETY: `wv_ptr` was captured from a live WKWebView owned by app state and this work runs on the main queue.
