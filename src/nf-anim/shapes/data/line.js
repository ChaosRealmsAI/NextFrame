...
const meta = {
  name: "line",
  category: "data",
  description: "Polyline chart line from layer.points",
  params: [
    {
      name: "points",
      type: "point[]",
...
function line(layer = {}) {
  ...
  const strokeWidth = layer.strokeWidth ?? 8;
  ...
  const progress = Math.max(
    ...
    Math.min(
      ...
      +(
        interp(
          layer.tracks?.strokeProgress || layer.tracks?.pathProgress,
          layer.t,
        ) ?? 1
      ),
    ),
  ...
  return `<polyline${attrs({
    points: pointsAttr(points, fallback),
    fill: "none",
    stroke,
    ...
    "stroke-dasharray": progress < 1 ? `${length} ${length}` : null,
    "stroke-dashoffset": progress < 1 ? length * (1 - progress) : null,
  })}/>`;
}
line.meta = meta;
export default line;
