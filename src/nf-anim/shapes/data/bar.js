...
const meta = {
  name: "bar",
  category: "data",
  description: "Single bar centered at the origin",
  params: [
    { name: "width", type: "number", default: 48, semantic: "bar width in px" },
    {
      name: "height",
      type: "number",
      default: 100,
      semantic: "bar height in px",
    },
    {
      name: "radius",
      type: "number",
...
function bar(layer = {}) {
  const width = Math.max(1, num(layer.width, 48));
  const height = Math.max(1, num(layer.height, 100));
  const radius = Math.max(0, num(layer.radius, 8));
