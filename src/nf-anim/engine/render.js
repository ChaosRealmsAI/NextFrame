...
const meta = {
  name: "renderMotion",
  kind: "engine",
  description: "Pure SVG renderer for motion configs",
};
...
function scalePair(layer, tracks, t) {
  const baseScale = Array.isArray(layer.scale)
    ? layer.scale
    : [num(layer.scale, 1), num(layer.scale, 1)];
  ...
  const sx = Array.isArray(trackScale)
    ? num(trackScale[0], 1)
    : num(trackScale, 1);
  const sy = Array.isArray(trackScale)
    ? num(trackScale[1], sx)
    : num(trackScale, 1);
  return [
...
function layerState(layer, t) {
  ...
  const absoluteAt =
    resolveTrack(tracks, "at", t) || resolveTrack(tracks, "position", t);
  const offset = resolveTrack(tracks, "offset", t) ||
    resolveTrack(tracks, "translate", t) || [
      num(resolveTrack(tracks, "x", t)),
      num(resolveTrack(tracks, "y", t)),
    ];
  const at = absoluteAt
    ? xy(absoluteAt, baseAt)
    : [baseAt[0] + num(offset[0], 0), baseAt[1] + num(offset[1], 0)];
  const rotate =
    num(layer.rotate, 0) + num(resolveTrack(tracks, "rotate", t), 0);
  const [scaleX, scaleY] = scalePair(layer, tracks, t);
  const opacity = Math.max(
    0,
    Math.min(
      1,
      num(layer.opacity, 1) * num(resolveTrack(tracks, "opacity", t), 1),
    ),
  );
...
function renderLayers(items, t, motion) {
  ...
  for (const item of items || [])
    for (const layer of expandLayer(item || {})) {
...
function renderShape(layer, state, t, motion) {
  const shapeName =
    layer.shape || (layer.type === "shape" ? "circle" : layer.type);
  const shape = SHAPES[shapeName];
  const d =
    shapeName === "path"
      ? resolveTrack(state.tracks, "d", t) || layer.path
      : layer.path;
  ...
  const fallback = d
    ? `<path${attrs({
        d,
        fill: resolvedLayer.fill ?? "#000000",
        stroke: resolvedLayer.stroke,
        "stroke-width": resolvedLayer.stroke ? (layer.strokeWidth ?? 1) : null,
        "vector-effect": "non-scaling-stroke",
      })}/>`
    : "";
  const ctx = motion.renderChildren
    ? motion
    : { ...motion, renderChildren: (items) => renderLayers(items, t, motion) };
  resolvedLayer.motion = ctx;
  const body = typeof shape === "function" ? shape(resolvedLayer) : fallback;
  if (!body) return "";
  const transform = [
    `translate(${state.at[0].toFixed(3)} ${state.at[1].toFixed(3)})`,
    `rotate(${state.rotate.toFixed(3)})`,
...
export function renderMotion(host = null, t = 0, motion = {}) {
  const size = Array.isArray(motion.size)
    ? motion.size
