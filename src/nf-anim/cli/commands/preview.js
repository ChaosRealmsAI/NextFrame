...
const meta = {
  name: "preview",
  kind: "cli",
  description: "Preview launcher stub",
};
const MIME = {
...
export default async function previewCmd() {
  ...
  const root = path.resolve(
    ...
    file = "/src/nf-anim/examples/gallery.html";
  const server = http.createServer((req, res) => {
    const raw = decodeURIComponent((req.url || "/").split("?")[0]),
      rel = raw === "/" ? file : raw,
      full = path.resolve(root, `.${rel}`);
    if (!full.startsWith(root))
      return void res.writeHead(403).end("Fix: path escapes repo root");
    try {
      res.writeHead(200, {
        "content-type": MIME[path.extname(full)] || "text/plain; charset=utf-8",
      });
      res.end(fs.readFileSync(full));
    } catch {
      res.writeHead(404).end(`Fix: missing ${rel}`);
    }
  });
