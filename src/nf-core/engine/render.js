...
export function renderAt(timeline, t, opts = {}) {
    ...
    if (needsResolve(timeline)) {
        ...
        if (!r.ok)
            return guarded("renderAt", { ok: false, error: r.error });
        resolved = r.value;
    }
    ...
    try {
        ...
        for (let ti = 0; ti < tracks.length; ti++) {
            ...
            if (trk.muted)
                continue;
            if (trk.kind === "audio")
                continue; // v0.1: ignore audio tracks for video render
            for (const clip of trk.clips || []) {
                ...
                if (blend === "source-over" && firstLayer && !hasEffects && !hasFilters) {
                    ...
                    try {
                        ...
                        if (hasFilters && offCtx)
                            applyFilters(offCtx, width, height, clip.filters ?? [], localT);
                    }
                    catch (err) {
                    ...
                    try {
                        ...
                        if (clip.effects?.enter)
                            applyEnterEffect(ctx, localT, clip.effects.enter, width, height);
                        if (clip.effects?.exit)
                            applyExitEffect(ctx, localT, dur, clip.effects.exit, width, height);
...
function needsResolve(timeline) {
    for (const trk of timeline.tracks ?? []) {
        for (const clip of trk.clips ?? []) {
            if (typeof clip.start !== "number" || typeof clip.dur !== "number")
                return true;
        }
    }
