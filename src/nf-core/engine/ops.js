...
export function addLayer(timeline, opts) {
    if (!opts.scene)
        return { ok: false, error: { code: 'MISSING_SCENE', message: 'opts.scene is required' } };
    ...
    if (opts.params)
        layer.params = opts.params;
    if (opts.enter)
        layer.enter = opts.enter;
    if (opts.exit)
        layer.exit = opts.exit;
    if (opts.blend)
        layer.blend = opts.blend;
