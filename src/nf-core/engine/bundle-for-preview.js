...
function normalizeRatio(timeline) {
...
function prepareDesignPreamble(source) {
...
function resolveScenePath(index, sceneId, preferredRatio) {
  const matches = index.get(sceneId) || [];
  if (!matches.length) fail(`scene "${sceneId}" was not found`);
  if (preferredRatio) {
    const exact = matches.find((entry) => entry.ratio === preferredRatio);
