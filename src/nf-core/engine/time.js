...
export function resolveExpression(expr, lookup, duration) {
    ...
    let value = base;
    if (op === "after")
        value = base + (exprObj.gap || 0);
    if (op === "before")
        value = base - (exprObj.gap || 0);
    if (op === "offset")
        value = base + (exprObj.by || 0);
...
function buildContext(timeline) {
    ...
    chapters.forEach((chapter, index) => {
        if (!chapter.id)
            throw withCode("MISSING_ID", `chapter[${index}] missing id`);
    ...
    markers.forEach((marker, index) => {
        if (!marker.id)
            throw withCode("MISSING_ID", `marker[${index}] missing id`);
    ...
    tracks.forEach((track, trackIndex) => {
        (track.clips ?? []).forEach((clip, clipIndex) => {
            if (!clip.id)
                throw withCode("MISSING_ID", `clip in track[${trackIndex}] missing id`);
...
function addNode(context, def) {
    ...
    if (def.path)
        context.paths.push({ nodeId: node.nodeId, path: def.path });
    context.refs.set(`${def.refBase}.${def.field}`, def.nodeId);
    if (def.field === "start" || def.field === "t") {
        if (!context.refs.has(def.refBase))
            context.refs.set(def.refBase, def.nodeId);
    }
}
...
function resolveRefId(context, ref, op) {
    if (context.refs.has(ref)) {
        ...
        if (op === "after" && (ref.startsWith("chapter-") || ref.startsWith("clip-"))) {
            ...
            if (context.refs.has(endKey))
                return context.refs.get(endKey);
        }
        return id;
    }
...
function topologicallySort(context) {
    const indeg = new Map();
    for (const node of context.nodes.values())
        indeg.set(node.nodeId, node.deps.length);
    const ready = [];
    for (const node of context.nodes.values()) {
        if (indeg.get(node.nodeId) === 0)
...
function resolveValues(nodes, order) {
    ...
    for (const id of order) {
        ...
        let value = ref;
        if (node.expr.op === "after")
            value = ref + node.expr.gap;
        else if (node.expr.op === "before")
            value = ref - node.expr.gap;
        else if (node.expr.op === "offset")
            value = ref + node.expr.by;
        values.set(id, value);
    }
    return values;
}
