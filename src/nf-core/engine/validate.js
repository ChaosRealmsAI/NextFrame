    ...
    for (const trk of resolved.tracks ?? []) {
        for (const clip of trk.clips ?? []) {
            ...
            for (const f of clip.filters ?? []) {
                const ft = typeof f === "object" && f !== null ? f.type : undefined;
