...
import { buildAnimationBundle, buildSceneBundle, readLegacyBundleSource, buildSharedPreamble, collectSceneModules, readDiscoveredScenes, } from "./build-scenes.js";
    ...
    for (const layer of timeline.layers || []) {
        ...
        for (const [key, val] of Object.entries(layer.params)) {
            if (typeof val === "string" && val.startsWith("[")) {
                try {
                    layer.params[key] = JSON.parse(val);
                }
                catch { /* keep string */ }
            }
        }
