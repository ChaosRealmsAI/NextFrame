...
export function getFilter(id) {
    const fn = FILTER_FNS[id];
    const meta = FILTER_TABLE[id];
    if (!fn)
        return null;
...
export function applyFilters(ctx, width, height, filters, t = 0, runtime = {}) {
    ...
    for (const spec of specs) {
        ...
        if (fn)
            fn(imageData.data, width, height, spec);
    }
...
function applyCssFilters(ctx, width, height, specs, createCanvas) {
    ...
    for (const spec of specs) {
        ...
        if (buildCss)
            filterChain.push(buildCss(spec));
    }
    ...
    for (const spec of specs) {
        if (spec.type === "filmGrain" && createCanvas)
            applyFilmGrainOverlay(ctx, width, height, spec, createCanvas);
    }
}
