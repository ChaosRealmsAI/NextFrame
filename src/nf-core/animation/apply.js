...
function resolveEasing(spec) {
    ...
    if (spec && typeof spec === "object") {
        ...
        if (type === "cubicBezier") {
            return cubicBezier(obj.x1, obj.y1, obj.x2, obj.y2);
        }
        if (type === "steps") {
...
export function getEffectCSS(effectName, progress, opts = {}) {
    const effectFns = EFFECT_FNS;
    const fn = effectFns[effectName] || EFFECT_FNS.fadeIn;
    return serializeStyle(fn(clamp01(progress), opts));
}
export function getTransitionCSS(transitionName, progress, opts = {}) {
    const transitionFns = TRANSITION_FNS;
    const fn = transitionFns[transitionName] || TRANSITION_FNS.dissolve;
