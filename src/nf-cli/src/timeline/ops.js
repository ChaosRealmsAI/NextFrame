...
export function findClips(timeline, predicate = {}) {
    const ids = [];
    for (const track of tracksOf(timeline)) {
        if (predicate.trackId && track.id !== predicate.trackId)
            continue;
        for (const clip of (track.clips || [])) {
            if (predicate.sceneId && clip.scene !== predicate.sceneId)
                continue;
            if (predicate.hasParam && !(predicate.hasParam.key in (clip.params || {})))
                continue;
            if (predicate.textContent) {
                const textValues = Object.values((clip.params || {})).filter((entry) => typeof entry === "string");
                if (!textValues.some((entry) => entry.includes(predicate.textContent)))
                    continue;
            }
            ids.push(clip.id);
        }
