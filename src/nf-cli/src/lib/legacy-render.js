...
export function renderAt(timeline, t, opts = {}) {
    ...
    if (needsResolve(timeline)) {
        ...
        if (!result.ok)
            return guarded("renderAt", { ok: false, error: result.error });
            ...
            for (const clip of track.clips || []) {
                ...
                if (blend === "source-over" && firstLayer && !hasEffects && !hasFilters) {
                    ...
                    try {
                        ...
                        if (hasFilters)
                            applyFilters(offscreenCtx, width, height, clip.filters ?? [], localT);
                    }
                    catch (error) {
                    ...
                    try {
                        ...
                        if (clip.effects?.exit)
                            applyExitEffect(ctx, localT, duration, clip.effects.exit, width, height);
