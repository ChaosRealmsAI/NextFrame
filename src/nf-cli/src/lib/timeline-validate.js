...
function registryLookup(sceneId, timelineRatio) {
    if (REGISTRY.has(sceneId))
        return REGISTRY.get(sceneId);
    // Try ratio:id key
    const prefixed = timelineRatio ? `${timelineRatio}:${sceneId}` : null;
    if (prefixed && REGISTRY.has(prefixed))
        return REGISTRY.get(prefixed);
    // Fall back: any ratio
...
export async function validateTimelineV3(timeline) {
    ...
    if (!timeline.width)
        errors.push({ code: "MISSING_FIELD", message: "width is required" });
    if (!timeline.height)
        errors.push({ code: "MISSING_FIELD", message: "height is required" });
    if (!timeline.fps)
        errors.push({ code: "MISSING_FIELD", message: "fps is required" });
    if (!timeline.duration)
        errors.push({ code: "MISSING_FIELD", message: "duration is required" });
    if (typeof timeline.duration === "number" && timeline.duration <= 0) {
        errors.push({ code: "BAD_DURATION", message: "duration must be > 0" });
    }
    if (!Array.isArray(timeline.layers))
        errors.push({ code: "MISSING_FIELD", message: "layers[] is required" });
    if (errors.length > 0)
    ...
    for (const layer of layers) {
        if (!layer.id)
            errors.push({ code: "MISSING_ID", message: "layer missing id" });
        if (!layer.scene)
            errors.push({ code: "MISSING_SCENE", message: `layer "${layer.id}" missing scene` });
        if (layer.start == null)
            errors.push({ code: "MISSING_START", message: `layer "${layer.id}" missing start` });
        if (layer.dur == null)
            errors.push({ code: "MISSING_DUR", message: `layer "${layer.id}" missing dur` });
    ...
    const contentLayers = layers.filter((layer) => {
        if (layer.scene && BG_SCENES.has(layer.scene))
            return false;
        if (layer.blend && layer.blend !== "normal")
            return false;
        if (layer.opacity != null && layer.opacity < 0.5)
            return false;
        if (hasExplicitBounds(layer))
            return false;
        return true;
    });
...
export function validateTimelineLegacy(timeline, opts = {}) {
    ...
    for (const track of resolved.tracks || []) {
        for (const clip of track.clips || []) {
            ...
            if (typeof start !== "number" || typeof clipDuration !== "number")
                continue;
    ...
    if (errors.length > 0)
        return guarded("validateTimeline", { ok: false, error: errors[0], errors, warnings, hints, resolved });
