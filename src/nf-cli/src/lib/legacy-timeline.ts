...
export function resolveTimeline(timeline: any) {
  try {
...
export function resolveExpression(expr: any, lookup: any, duration: any) {
...
function buildContext(timeline: any) {
  const context = {
    timeline,
    nodes: new Map(),
    refs: new Map(),
  ...
  tracks.forEach((track: any, trackIndex: any) => {
    (track.clips || []).forEach((clip: any, clipIndex: any) => {
...
function addNode(context: any, def: any) {
  const node = {
    nodeId: def.nodeId,
    refBase: def.refBase,
    field: def.field,
...
function buildDependencyGraph(context: any) {
...
function resolveRefId(context: any, ref: any, op: any) {
...
function listRefs(context: any) {
  return `available: ${[...context.refs.keys()].slice(0, 12).join(", ")}`;
}

function topologicallySort(context: any) {
  const indeg = new Map();
...
function resolveValues(nodes: any, order: any) {
...
function quantizeAll(values: any) {
  for (const [key, value] of values.entries()) {
    values.set(key, quantize(value));
  }
}

function quantize(value: any) {
  return Number((Math.round((value + 1e-9) * 10) / 10).toFixed(1));
}

function rangeCheck(nodes: any, values: any, duration: any) {
...
function materialize(context: any, values: any) {
...
function buildLookup(context: any, values: any) {
  const lookup = {};
...
function withCode(code: any, message: any, ref: any, hint: any) {
