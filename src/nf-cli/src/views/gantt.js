...
function fmtTime(seconds, withTenths) {
 ...
 */
export function renderGantt(timeline) {
...
function normalize(timeline) {
    ...
    const tracks = (timeline.tracks || []).map((trk, i) => {
...
function createLayout(t) {
    const trackLabelWidth = clamp(Math.max(4, ...t.tracks.map((tr) => String(tr.id).length)), 4, 6);
    const sideLabel = clamp(Math.max(0, ...t.tracks.flatMap((tr) => tr.clips.map((c) => c.label.length))), 0, 16);
...
function pickTickStep(duration) {
    ...
    for (let e = -2; e <= 5; e++) {
        const base = 10 ** e;
        for (const m of [1, 2, 2.5, 5, 10, 15, 20, 30])
            cands.push(base * m);
    }
    let best = cands[0];
...
function posFor(t, duration, w) {
    if (w <= 1 || duration <= 0)
        return 0;
    return clamp(Math.round((t / duration) * (w - 1)), 0, w - 1);
}
function place(buf, start, text) {
...
function compose(left, chart, layout, right) {
...
function chapterLines(t, layout) {
    ...
    for (const ch of t.chapters) {
        ...
        const center = clamp(s + Math.floor((span - text.length) / 2), s, Math.max(s, e - text.length + 1));
...
function axisLines(t, layout, ticks) {
    ...
    const placements = ticks.map((tk, i) => {
        ...
        if (i === 0)
            ...
            s = layout.chartWidth - lbl.length;
    ...
    for (let i = 1; i < placements.length - 1; i++) {
        ...
        if (c.start > prev.end + 1 && c.end < last.start - 1)
            sel.push(c);
    }
    if (last.i !== placements[0].i)
        sel.push(last);
    ticks.forEach((tk, i) => {
        const p = posFor(tk, t.duration, layout.chartWidth);
        if (i === 0)
