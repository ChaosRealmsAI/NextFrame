...
export async function run(argv: any) {
  ...
  const atoms = flags.type ? pipeline.atoms.filter((atom: any) => atom.type === flags.type) : pipeline.atoms;
  if (flags.json) {
...
function renderTable(atoms: any) {
  if (atoms.length === 0) return "(no atoms)";
  return formatTable(
    ["ID", "TYPE", "NAME", "DETAILS"],
    atoms.map((atom: any) => [String(atom.id), String(atom.type), String(atom.name), describeAtom(atom)])
  );
}

function describeAtom(atom: any) {
