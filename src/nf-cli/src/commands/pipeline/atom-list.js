...
function renderTable(atoms) {
    if (atoms.length === 0)
        return "(no atoms)";
    return formatTable(["ID", "TYPE", "NAME", "DETAILS"], atoms.map((atom) => [String(atom.id), String(atom.type), String(atom.name), describeAtom(atom)]));
}
function describeAtom(atom) {
    if (atom.type === "component")
        return `${atom.scene} seg=${atom.segment}`;
    if (atom.type === "video")
        return `${atom.file} dur=${atom.duration}`;
