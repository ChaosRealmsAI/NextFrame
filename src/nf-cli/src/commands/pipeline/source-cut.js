...
import { assertSourceBinAvailable, buildClipsFromCut, ensureDirectory, fail, looksLikeSourcePath, parseSourceFlags, readJson, readSourceJson, resolveEpisodeSourceDir, resolveSourceBin, runSourceBinary, success, writeSourceJson, } from "../_helpers/_source.js";
...
function normalizeCutReport(cutReport, clipsDir) {
    ...
    const rows = Array.isArray(cr)
        ? cr
        : Array.isArray(cr?.success)
            ? cr.success
            : Array.isArray(cr?.clips)
                ? cr.clips
                : [];
