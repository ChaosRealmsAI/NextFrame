// Reads pipeline audio settings or a single audio segment from an episode.
...
export async function run(argv: string[]) {
  ...
  const value = segmentNumber === undefined
    ? pipeline.audio
    : pipeline.audio.segments.find((segment: Record<string, unknown>) => Number(segment.segment) === segmentNumber) || null;
