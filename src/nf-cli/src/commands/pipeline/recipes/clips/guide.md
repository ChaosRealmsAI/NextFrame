...
长视频 → 挑亮点 → 切片 → 多语言字幕 → 传播文案。每步有提示词，跑 `nextframe clips <step>` 获取。
...
| 步骤 | 命令 | 谁做 | 产物（gate） |
|------|------|------|--------------|
| 0 | `nextframe clips download` | Code | `sources/<slug>/source.mp4` + `meta.json` |
| 1 | `nextframe clips transcribe` | Code | `sources/<slug>/sentences.json` + `words.json` |
| 2 | `nextframe clips plan` | **Agent** | `plan.json` |
| 3 | `nextframe clips cut` | Code | `clips/clip_NN.mp4` + `cut_report.json` |
| 4 | `nextframe clips translate` | **Agent** | `clips/clip_NN.translations.<lang>.json` |
| 5 | `nextframe clips polish` | **Agent** | `clips/clip_NN.caption.<lang>.md` |
| 6 | `nextframe clips publish` | Code | `clips/clip_NN.publish.json` |
...
1. `nextframe clips` 无参数 → 看这份 guide，知道整体流程
2. `nextframe clips <step>` → 拿到那步的完整提示词
3. 按提示词干活，写产物
4. 下一步再跑 `nextframe clips <next-step>`
