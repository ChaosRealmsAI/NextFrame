...
export async function run(argv) {
    ...
    for (const t of times) {
        ...
        const fullscreenContent = analysis.visible.filter((v) => v.isFullscreen && Number(v.opacity) > 0.3 && !BG_IDS.test(String(v.id)));
    ...
    if (flags.json) {
        ...
        for (const s of screenshots) {
            ...
            for (const v of s.visible) {
                ...
                const extra = [];
                if (v.blend !== "normal")
                    extra.push(`blend:${v.blend}`);
                if (v.opacity < 1)
                    extra.push(`α:${v.opacity}`);
