...
export async function run(argv) {
        ...
        switch (sub) {
            case "navigate": {
                if (typeof flags.project !== "string" || !flags.project.trim()) {
                    emit({ ok: false, error: { code: "USAGE", message: "usage: nextframe app-pipeline navigate --project=<name> [--episode=<name>]" } }, flags);
                    return 3;
                }
            ...
            case "tab": {
                if (typeof flags.tab !== "string" || !flags.tab.trim()) {
                    emit({ ok: false, error: { code: "USAGE", message: "usage: nextframe app-pipeline tab --tab=<name>" } }, flags);
                    return 3;
                }
            ...
            case "play": {
                ...
                if (!Number.isInteger(index) || index < 0) {
                    emit({ ok: false, error: { code: "USAGE", message: "usage: nextframe app-pipeline play --segment=<n>" } }, flags);
                    return 3;
                }
                const script = `(() => {
  ...
  return { segment: ${index + 1}, ok: true };
            ...
            case "stop": {
                const result = await httpPost("/eval", "document.querySelectorAll('.pl-play-btn.playing').forEach((button) => button.click())");
                emit({ ok: true, value: result }, flags);
                return 0;
            }
            default:
...
function request(path, options) {
    ...
    const body = typeof rawBody === "string" ? rawBody : rawBody === undefined ? "" : JSON.stringify(rawBody);
    const headers = {};
    if (body) {
