...
export async function run(argv) {
    ...
    try {
        if (subcommand === "eval")
            return await runEval(argv.slice(1));
        if (subcommand === "screenshot")
            return await runScreenshot(argv.slice(1));
        if (subcommand === "diagnose")
            return await runDiagnose(argv.slice(1));
        if (subcommand === "navigate")
            return await runNavigate(argv.slice(1));
        if (subcommand === "click")
            return await runClick(argv.slice(1));
        if (subcommand === "status")
            return await runStatus(argv.slice(1));
