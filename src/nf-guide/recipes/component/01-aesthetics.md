...
| 类型 | 最低占比 | 用什么实现 |
|------|---------|------|
| 带图形主体的组件 | ≥ 50% | SVG / Canvas / CSS / 混合都行，但都写在 dom 里 |
| 带持续动态的组件 | ≥ 20% | t-driven opacity / transform / draw / filter |
| 纯 DOM 文字组件 | ≤ 30% | 标题 / 金句 / chrome |
| 数据可视化 | ≥ 10% | 图表 / 数字 / 路径 / 指示器 |

纯文字组件 > 50% = 主题质量不及格，重做。

### 8.2 技术只是实现手段

别再问"该不该发明一个新技术 type"。v0.9.3 之后，问题只剩：

- 这个主体最适合写成什么 HTML 组合
- 它是不是 frame-pure 的 t-driven
- 它在 1x gallery 缩略图里是否一眼可读

### 8.3 持续动画（idle motion）铁律
...
- §5 动画 → 8.3 强化为"必须有 idle motion 不只入场"
- 技术选型 → 从"选 type"改成"在 dom 里选实现手段"
