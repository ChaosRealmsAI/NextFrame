...
- v2.0 最终架构见 `spec/versions/v2.0/architecture/` (ADR-023 + ADR-024 + crates.json + v20-rebuild.walkthrough)；v1.0 时期的北极星原稿 `spec/versions/v1.0/pm-docs/ultimate-architecture.html`。
...
1. 先读 roadmap：`cat spec/roadmap.json` → 确认当前版本阶段（v2.0 当前在 architecture）
2. 再读架构决策：`spec/versions/v2.0/architecture/crates.json` + `spec/adrs.json` 里的 ADR-023 / ADR-024
3. 参考原北极星：`spec/versions/v1.0/pm-docs/ultimate-architecture.html`
...
| `nf-core-engine` | Rust (+WASM for JS) | Source → Resolved 编译 + bundle + byte-idempotent writeBack (ADR-023 升级) |
...
| `nf-recorder` | Rust | CARenderServer → IOSurface (零拷贝) → Metal Heap + tile shaders → VTCompressionSession HEVC 10-bit HDR10 → fragmented MP4 (ADR-023) |
