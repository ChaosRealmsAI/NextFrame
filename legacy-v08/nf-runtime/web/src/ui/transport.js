...
function iconButton(id, title, action, markup) {
    return h('button', { class: 'ed-t-btn' + (id === 'ed-btn-play' ? ' play-main' : ''), id, type: 'button', title, onclick: action }, createMarkupNode(markup));
}
function updatePlayButton(playing) {
    const mainBtn = document.getElementById('ed-btn-play');
    const heroBtn = document.querySelector('.ed-play-btn');
    if (mainBtn)
        mainBtn.innerHTML = playing ? pauseIconMarkup : playIconMarkup;
    if (heroBtn)
        heroBtn.innerHTML = playing ? heroPauseIconMarkup : heroPlayIconMarkup;
}
...
function sendPreviewCmd(action, time) {
    ...
    if (!engine)
        return;
    if (action === 'play' && typeof engine.play === 'function')
        engine.play();
    else if (action === 'pause' && typeof engine.pause === 'function')
        engine.pause();
    else if (action === 'toggle') {
        if (typeof engine.toggle === 'function')
            engine.toggle();
        else if (edPlaybackState.isPlaying && typeof engine.pause === 'function')
            engine.pause();
        ...
        if (typeof engine.seek === 'function')
            engine.seek(time);
        else if (typeof engine.compose === 'function')
            engine.compose(time);
...
class TransportControls extends Component {
    didMount() {
        ...
        const heroBtn = document.querySelector('.ed-play-btn');
        if (heroBtn)
            heroBtn.onclick = () => sendPreviewCmd('toggle');
        updatePlayButton(false);
    }
    render() {
        return h('div', { class: 'ed-transport' }, iconButton('ed-btn-start', 'Previous', () => sendPreviewCmd('seek', 0), '<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="2" y="3" width="2" height="10" rx="0.5" fill="currentColor"/><path d="M13 3.5L6 8l7 4.5V3.5z" fill="currentColor"/></svg>'), iconButton('ed-btn-back5', '-5s', () => sendPreviewCmd('seek', Math.max(0, edPlaybackState.currentTime - 5)), '<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M9 3L2 8l7 5V3z" fill="currentColor"/><path d="M15 3L8 8l7 5V3z" fill="currentColor"/></svg>'), iconButton('ed-btn-play', 'Play / Pause', () => sendPreviewCmd('toggle'), playIconMarkup), iconButton('ed-btn-fwd5', '+5s', () => sendPreviewCmd('seek', edPlaybackState.currentTime + 5), '<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M7 3l7 5-7 5V3z" fill="currentColor"/><path d="M1 3l7 5-7 5V3z" fill="currentColor"/></svg>'), iconButton('ed-btn-end', 'Next', () => sendPreviewCmd('seek', edPlaybackState.duration), '<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M3 3.5l7 4.5-7 4.5V3.5z" fill="currentColor"/><rect x="12" y="3" width="2" height="10" rx="0.5" fill="currentColor"/></svg>'), h('span', { class: 'ed-transport-tc' }, '00:00.0'), h('div', { class: 'ed-transport-progress' }, h('div', { class: 'ed-transport-fill' }), h('div', { class: 'ed-transport-thumb' })));
    }
}
