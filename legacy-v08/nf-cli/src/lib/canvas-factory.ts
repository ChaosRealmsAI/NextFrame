...
interface CanvasFontReport {
  searchedRoots: string[];
  matchedFonts: string[];
  aliasAssignments: Record<string, string>;
}

const FONT_EXTENSIONS = new Set([".ttf", ".ttc", ".otf", ".otc"]);
const FONT_PATTERNS: Record<FontKind, string[]> = {
  mono: [
    "jetbrains mono",
    "jetbrainsmono",
    "sf mono",
...
const FONT_ALIASES: Record<FontKind, string[]> = {
...
const DEFAULT_REPORT: CanvasFontReport = {
  searchedRoots: [],
  matchedFonts: [],
  aliasAssignments: {},
};
...
function normalizeFontName(filePath: string) {
...
function tokenMatchIndex(filePath: string, tokens: string[]) {
...
function classifyFont(filePath: string): { kind: FontKind; tokenIndex: number } | null {
  let best: { kind: FontKind; tokenIndex: number } | null = null;
  for (const kind of ["mono", "serif", "sans"] as FontKind[]) {
...
function scanFontDirectory(root: string, rootIndex: number, candidates: FontCandidate[]) {
...
function sortCandidates(a: FontCandidate, b: FontCandidate) {
...
function registerFontPath(filePath: string, alias?: string) {
  try {
...
export function ensureCanvasFonts() {
  if (fontsInitialized) return cachedReport;
  ...
  const aliasAssignments: Record<string, string> = {};
  for (const kind of ["sans", "serif", "mono"] as FontKind[]) {
