...
const meta = {
  name: "text",
  category: "text",
  description: "Centered SVG text primitive",
  params: [
    { name: "text", type: "string", default: "Text", semantic: "text content" },
    {
      name: "fontSize",
      type: "number",
      default: 48,
...
function text(layer = {}) {
  ...
  const v = Number.isFinite(n)
    ? `${layer.prefix ?? ""}${Number(n).toFixed(d).replace(/\.0+$/, "")}${layer.suffix ?? ""}`
    : (layer.text ?? "Text");
  const fontSize = num(layer.fontSize, 48);
