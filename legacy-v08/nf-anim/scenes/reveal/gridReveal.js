...
const FALLBACK = [
  { fill: "#d97b57", label: "Warm" },
  { fill: "#f1b787", label: "Signal" },
  { fill: "#f7dfc7", label: "Soft" },
  { fill: "#86b3a2", label: "Calm" },
  { fill: "#5d7e8e", label: "Depth" },
  { fill: "#2c4251", label: "Anchor" },
];
...
const norm = (items) =>
  (Array.isArray(items) && items.length ? items : FALLBACK).map((item, i) =>
    typeof item === "object" && item
      ? {
          fill: item.fill || item.color || COLORS[i % COLORS.length],
          label: item.label || item.title || "",
        }
      : /^#|rgb|hsl/.test(String(item || ""))
        ? { fill: String(item), label: "" }
        : { fill: COLORS[i % COLORS.length], label: String(item || "") },
  );
const meta = {
  id: "gridReveal",
  ratio: "any",
  ...
  category: "reveal",
  description:
    "Color tile grid that reveals cell by cell with direct stagger timing",
  params: [
    { name: "items", type: "array", default: FALLBACK },
    { name: "cols", type: "number", default: 3 },
    { name: "stagger", type: "number", default: 0.1 },
  ],
  examples: [{ items: FALLBACK, cols: 3, stagger: 0.08 }],
};
export default {
  ...meta,
  render(host, t, params = {}, vp = { width: 0, height: 0 }) {
    ...
    const stagger = Number.isFinite(params.stagger) ? params.stagger : 0.1;
    const W = vp.width || 1920;
    const H = vp.height || 1080;
    const cellW = (W * 0.66) / cols;
    const cellH = (H * 0.54) / rows;
    const tileW = cellW * 0.9;
    const tileH = cellH * 0.9;
    ...
    items.forEach((item, i) => {
      ...
      layers.push({
        ...
        at: [x, y + tileH * 0.24],
        text: item.label,
        fontSize: Math.min(24, tileW * 0.11),
        font: "Avenir Next, system-ui, sans-serif",
        fill: "#fff7f2",
        behaviors: [slideInUp(start + 0.08, 0.3, { distance: 18 })],
      });
    });
    return {
      duration: Math.max(
        meta.duration_hint,
        +(0.95 + items.length * stagger + 0.65).toFixed(2),
      ),
      size: [W, H],
      layers,
    };
