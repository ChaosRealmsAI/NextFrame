const meta = {
  name: "color",
  kind: "engine",
  description: "Color parsing and interpolation helpers",
};
const CACHE = new Map();
const HEX = /^#([0-9a-f]{3}|[0-9a-f]{6}|[0-9a-f]{8})$/i;
const FUNC = /^(rgba?|hsla?)\((.*)\)$/i;
const NAMED = {
  aqua: [0, 255, 255, 1, true],
  black: [0, 0, 0, 1, false],
  blue: [0, 0, 255, 1, false],
  cyan: [0, 255, 255, 1, true],
  fuchsia: [255, 0, 255, 1, true],
  gray: [128, 128, 128, 1, false],
  green: [0, 128, 0, 1, false],
  grey: [128, 128, 128, 1, false],
  lime: [0, 255, 0, 1, false],
  magenta: [255, 0, 255, 1, true],
  maroon: [128, 0, 0, 1, false],
  navy: [0, 0, 128, 1, false],
  orange: [255, 165, 0, 1, false],
  purple: [128, 0, 128, 1, false],
  red: [255, 0, 0, 1, false],
  silver: [192, 192, 192, 1, false],
  transparent: [0, 0, 0, 0, true],
  white: [255, 255, 255, 1, false],
  yellow: [255, 255, 0, 1, false],
};
const clamp = (value, min, max) => Math.max(min, Math.min(max, value));
const round = (value) => Math.round(clamp(value, 0, 255));
const alpha = (value) => clamp(Number.isFinite(value) ? value : 1, 0, 1);
const color = (r, g, b, a = 1, hasAlpha = false) => ({
  r: round(r),
  g: round(g),
  b: round(b),
  a: alpha(a),
...
const clone = (value) =>
  value ? { r: value.r, g: value.g, b: value.b, a: value.a } : null;
...
export function lerpColor(left, right, t) {
  ...
  return formatColor(
    ...
    from.hasAlpha || to.hasAlpha || from.a < 1 || to.a < 1,
  );
}
